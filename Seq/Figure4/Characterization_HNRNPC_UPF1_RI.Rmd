---
title: "Characterization of RI events regulated by both UPF1 and HNRNPC"
author: "Luca Ducoli"
...
write.table(sign.regions, file = "~/Documents/Postdoc/PD_Projects/3_irCLIP-RNP/MS/siRNA_EGF_SG/2_EGF/8_DTE_analysis/0_UPF1_HNRNPC/0_Overlap/HNRNPC_UPF1_RIevents.txt", quote = F, row.names = F, sep = "\t")
```
...
pdf("~/Documents/Postdoc/PD_Projects/3_irCLIP-RNP/MS/siRNA_EGF_SG/2_EGF/8_DTE_analysis/0_UPF1_HNRNPC/0_Overlap/GO_AS.pdf", height = 8, width = 6.5)
ggplot(data = gp.bp, aes(x = source, y = term_name, color = -log10(p_value), size = intersection_size)) + 
  geom_point() +
  scale_color_viridis(option = "D") +
  theme_bw() + 
...
After Salmon quantification (PMID:28263959), we performed differential transcript analysis using the Swish R package (PMID:31372651). We then compared the logFC trend of significant deregulated RI transcripts vs the other signficant DE transcripts.
```{r warning=FALSE, message=FALSE}
...
se <- labelKeep(se)
se <- se[mcols(se)$keep,]
write.table(assay(se), file = "~/Documents/Postdoc/PD_Projects/3_irCLIP-RNP/MS/siRNA_EGF_SG/2_EGF/8_DTE_analysis/0_UPF1_HNRNPC/1_DTE/DTE_norm_counts.txt", quote = FALSE, sep = "\t", row.names = T)
```

```{r warning=FALSE, message=FALSE, results='hide'}
set.seed(1)
#Prepare annotation - just use the following code only once and saveDb will store the annotation as a database file that can be accessed later with loadDb
...
```{r message=FALSE, warning=FALSE, results='hide'}
#Save the plot as pdf
pdf("~/Documents/Postdoc/PD_Projects/3_irCLIP-RNP/MS/siRNA_EGF_SG/2_EGF/8_DTE_analysis/0_UPF1_HNRNPC/1_DTE/ECDF_DTE_HNRNPC_UPF1.pdf", height = 5, width = 9)
cdf
dev.off()
```
